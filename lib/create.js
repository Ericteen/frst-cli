...
module.exports = async function create (projectName) {
  ...
  if (fs.existsSync(targetDir)) {
    ...
    if (!action) {
      ...
      try {
        ...
        console.log(`\n${chalk.green(targetDir)} removed successfully`)
      } catch (err) {
        console.error(`Remove directory/file error: ${chalk.red(err)}`)
      }
    }
  }
  ...
  downloadRemote(remoteUrl, projectName).then(res => {
    ...
    spinner.fail(chalk.red('Sorry, something went wrong, please check it out. \n'))
    process.exit(-1)
  })
}
